...
<head>
    ...
    <script>
        ...
        <sec:authorize access="!hasRole('ROLE_ANONYMOUS')">
             window.frontendConfig.userName = ${principal};
            <c:choose>
                <c:when test="${authenticationMethod == 'saml'}">
                     window.frontendConfig.logoutUrl = ${samlLogoutUrl};
                </c:when>
                <c:otherwise>
                     window.frontendConfig.logoutUrl = j_spring_security_logout;
                </c:otherwise>
            </c:choose>
        </sec:authorize>
        ...
        <sec:authorize access="hasRole('ROLE_ANONYMOUS')">
            window.frontendConfig.googleLogin = true; 
        </sec:authorize>
